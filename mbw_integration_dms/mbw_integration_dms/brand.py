...
from mbw_integration_dms.mbw_integration_dms.product import publish, key_realtime_product

key_realtime_categories = "dms.key.sync.all.categories"
# Đồng bộ danh sách nhãn hiệu 
def sync_brand():
    frappe.enqueue("mbw_integration_dms.mbw_integration_dms.brand.sync_brand_job", queue="long", timeout=300, key=key_realtime_categories)
...
def sync_brand_job(*args, **kwargs):
    try:
        ...
        if not brands:
            ...
            publish(key_realtime_categories, "No new brand to sync.", done=True)
        ...
        if response.get("status"):
            ...
            publish(key_realtime_categories, "Brand synced successfully.", done=True)
