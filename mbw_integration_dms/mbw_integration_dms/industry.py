...
def sync_industry():
    frappe.enqueue("mbw_integration_dms.mbw_integration_dms.industry.sync_industry_job", queue="long", timeout=300, key=key_realtime_categories)
...
def sync_industry_job(*args, **kwargs):
    try:
        ...
        if not industrys:
            ...
            publish(key_realtime_categories, "No new industry to sync.", done = True)
        ...
        if response.get("status"):
            ...
            publish(key_realtime_categories, "Industry synced successfully.", done=True)
